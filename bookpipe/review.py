...
header{flex:0 0 auto;z-index:10;background:rgba(255,255,255,.98);border-bottom:1px solid var(--line);padding:14px 20px 12px}
.topline{display:flex;gap:18px;align-items:baseline;flex-wrap:wrap}.topline h1{font-size:21px;margin:0}.stats{font-size:13px;color:var(--muted)}
.progress{height:5px;background:#e7e7eb;border-radius:999px;margin:10px 0 12px;overflow:hidden}.progress>div{height:100%;background:#2b2b2d;width:0;transition:width .2s}
.controls{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.search{min-width:250px;flex:1;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:#fff}
.chips{display:flex;gap:6px;flex-wrap:wrap;margin-top:9px}.chip,.filter{border:1px solid var(--line);background:#fff;border-radius:999px;padding:5px 9px;font-size:12px}.chip.active,.filter.active{background:#222;color:#fff;border-color:#222}
.chip.complete,.filter.complete{color:var(--ok);border-color:#b9d5c4}.chip.complete.active,.filter.complete.active{color:var(--ok);background:#eaf5ee;border-color:var(--ok);box-shadow:inset 0 0 0 1px var(--ok)}.chip.zero{color:#999}.viewline{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:9px}.viewline .hint{margin:0}.viewline button{margin-left:auto}.viewnotice{font-size:12px;color:var(--ok)}.viewnotice.error{color:#a32424}.scope-count{font-variant-numeric:tabular-nums}.bulk:disabled{opacity:.45;cursor:not-allowed}
.layout{flex:1;min-height:0;display:grid;grid-template-columns:minmax(330px,38%) minmax(430px,1fr)}
.list{min-height:0;overflow:auto;border-right:1px solid var(--line);background:#fff}.termrow{padding:10px 14px;border-bottom:1px solid #eee;cursor:pointer;display:grid;grid-template-columns:1fr auto;gap:8px}.termrow:hover{background:#f7f7f9}.termrow.active{background:#ededf1}.termname{font-weight:650}.termmicro{font-size:12px;color:var(--muted);margin-top:3px}.mark{font-size:12px;white-space:nowrap}.reviewed{color:var(--ok)}.pending{color:#888}.uncertain{color:var(--warn)}.hasnote{color:var(--info);font-weight:650}
.detail{min-height:0;overflow:hidden;padding:22px 28px}.card{max-width:980px;height:100%;margin:0 auto;background:var(--panel);border:1px solid var(--line);border-radius:12px;box-shadow:0 1px 3px rgba(0,0,0,.03);display:flex;flex-direction:column;overflow:hidden}.cardbody{flex:1;min-height:0;overflow:auto;padding:22px}
.detailhead{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.detail h2{margin:0;font-size:25px}.badges{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.badge{font-size:11px;text-transform:uppercase;letter-spacing:.04em;border-radius:999px;background:var(--soft);padding:5px 7px;color:#555}.badge.low,.badge.medium{color:var(--warn)}
.snapshot{margin-top:18px;border:1px solid #e2e2e7;background:#fafafd;border-radius:10px;padding:12px}.snapshot-title{font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin-bottom:9px}.glance-grid{display:flex;gap:8px;flex-wrap:wrap}.glance{display:flex;align-items:center;gap:7px;border:1px solid #dedee5;border-radius:9px;background:#fff;padding:7px 10px;min-height:38px}.glance-icon{font-size:19px;line-height:1}.glance-label{font-weight:650;font-size:13px}.glance-sub{font-size:11px;color:var(--muted);margin-top:1px}.facts{margin-top:10px;border-top:1px solid #e5e5ea;padding-top:7px}.fact{display:grid;grid-template-columns:24px 1fr auto;gap:7px;align-items:start;padding:4px 0;font-size:13px}.fact-icon{font-size:16px;text-align:center}.fact-conf{font-size:10px;text-transform:uppercase;color:var(--muted);padding-top:2px}
.section{margin-top:20px}.section h3{font-size:13px;text-transform:uppercase;letter-spacing:.05em;color:#5b5b63;margin:0 0 8px}.note{line-height:1.48}.aliases{color:var(--muted)}
.option{display:block;border:1px solid var(--line);border-radius:9px;padding:10px 12px;margin:7px 0;background:#fff}.option.selected{border-color:#8a8a91;background:#fafafa}.optionline{display:flex;gap:9px;align-items:flex-start}.optiontext{font-weight:650}.reason{font-size:12px;color:var(--muted);margin:4px 0 0 26px;line-height:1.4}.confidence{font-size:11px;color:var(--muted);margin-left:auto;text-transform:uppercase}
.customrow{display:flex;gap:8px;align-items:center;margin-top:9px}.customrow input{flex:1;border:1px solid var(--line);border-radius:8px;padding:9px 10px}.smallbtn{border:1px solid var(--line);background:#fff;border-radius:8px;padding:8px 10px;font-size:12px}
.reviewnotes textarea{width:100%;min-height:92px;resize:vertical;border:1px solid var(--line);border-radius:9px;padding:10px 11px;line-height:1.45;background:#fff}.hint{font-size:11px;color:var(--muted);margin-top:5px}
.evidence details{border-top:1px solid #eee;padding:9px 0}.evidence summary{cursor:pointer;font-size:13px;color:#444}.excerpt{white-space:pre-wrap;line-height:1.5;margin:8px 0;color:#2b2b2d}.excerpt mark{background:#fff1a8}.meta{font-size:11px;color:var(--muted)}
.actions{flex:0 0 auto;padding:12px 22px;background:#fff;border-top:1px solid var(--line);display:flex;gap:8px;align-items:center;min-height:58px}.primary{border:0;background:#202124;color:#fff;border-radius:8px;padding:9px 13px;font-weight:650}.primary{display:inline-flex;align-items:center;justify-content:center;gap:8px;white-space:nowrap}.primary:disabled{opacity:.4;cursor:not-allowed}.state{font-size:12px;color:var(--muted);margin-left:auto}.confirm{border:1px solid #777;background:#fff;border-radius:8px;padding:8px 11px;font-size:12px}.confirm.ready{border-color:var(--ok);color:var(--ok)}
...
<body>
<header>
  <div class="topline"><h1>Terminology Review</h1><div class="stats" id="stats">Loading…</div><button class="confirm" id="confirmBtn">Confirm glossary</button></div>
