...
test('P1 prices update with recorded usage during a run and stop pulsing when it ends', { timeout: 60000 }, async () => {
  ...
  try {
    ...
    try {
      ...
      const costDetails = page.locator('[data-ui-debug-id="PUC"] details')
      assert.equal(await costDetails.evaluate(node => node.open), false, 'P1 costs and tokens start collapsed')
      await costDetails.locator('summary').click()
      await page.locator('[data-ui-debug-id="PUC"]').getByRole('heading', { name: 'Current model assignments' }).waitFor()
      await page.locator('[data-ui-debug-id="PUC"]').getByRole('heading', { name: 'Recorded provider usage' }).waitFor()
      assert.equal(await page.locator('[data-ui-debug-id="PUC"] .analyse-model-grid tbody tr').count(), 1)
      assert.equal(await page.locator('[data-ui-debug-id="PUC"] .pass-summary tbody td').first().textContent(), 'P1')
      assert.equal(await page.locator('[data-ui-debug-id="PUC"] th').last().textContent(), 'Cost')
      assert.equal(await page.locator('[data-ui-debug-id="PUC"] tbody td').last().textContent(), '$0.1235')
      assert.equal(await page.locator('[data-ui-debug-id="PUC"] tbody td').last().evaluate(node => node.scrollWidth <= node.clientWidth), true,
        'the Cost value is not ellipsized on desktop')
      await costDetails.locator('summary').click()
      assert.equal(await page.locator('[data-ui-debug-id="PAN"] .analyse-unit-table th').count(), 2)
      await page.locator('[data-ui-debug-id="PAV"]').getByText('Ada visits the town.').waitFor()
      const layout = await page.evaluate(() => {
        const heading = document.querySelector('[data-ui-debug-id="PHH"]').getBoundingClientRect()
        ...
        const costs = document.querySelector('[data-ui-debug-id="PUC"]').getBoundingClientRect()
        const grid = document.querySelector('.analyse-work-grid').getBoundingClientRect()
        const recent = document.querySelector('[data-ui-debug-id="RAL"]').getBoundingClientRect()
        ...
        return { headingBottom: heading.bottom, analysisTop: analysis.top, analysisBottom: analysis.bottom,
          previewTop: preview.top, previewRight: preview.right, previewHeight: preview.height,
          costsTop: costs.top, costsBottom: costs.bottom, costsWidth: costs.width, gridWidth: grid.width, recentTop: recent.top,
          analysisWidth: analysis.width, analysisHeight: analysis.height,
          mainWidth: main.width - parseFloat(mainStyle.paddingLeft) - parseFloat(mainStyle.paddingRight) }
      })
      assert.ok(layout.headingBottom < layout.analysisTop, 'whole-book heading precedes the table')
      assert.ok(layout.recentTop > layout.costsBottom, 'recent execution follows the costs panel')
      assert.ok(Math.abs(layout.analysisTop - layout.previewTop) < 1, 'unit list and P1 preview share a row')
      assert.ok(Math.abs(layout.analysisHeight - layout.previewHeight) < 1, 'unit list and preview share viewport height')
      assert.ok(layout.costsTop > layout.analysisBottom, 'costs and tokens form a separate row below the unit list')
      assert.ok(Math.abs(layout.costsWidth - layout.gridWidth) < 1, 'costs and tokens fill the available width')
      ...
      pipeline.review.summary = { total: 2, reviewed: 0, unreviewed: 2, uncertain: 0, confirmed: false, categories: {} }
      ...
      await page.getByText(/Whole-book analysis.*2 entities/).waitFor()
