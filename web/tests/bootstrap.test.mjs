...
test('production same-origin offline workflow, Review, Reader, archive, themes and responsive smoke', {timeout:180000}, async t => {
 ...
 await page.getByRole('button',{name:'Review & next',exact:true}).waitFor()
 ...
 await wait(async()=>await page.getByRole('button',{name:'Confirm glossary',exact:true}).isEnabled(),'review acknowledged')
 await page.getByRole('button',{name:'Confirm glossary',exact:true}).click()
 await page.getByText('✓ Glossary confirmed',{exact:true}).waitFor()
 ...
 await page.getByRole('button',{name:'Review & next',exact:true}).click()
 await page.getByRole('button',{name:'Confirm glossary',exact:true}).click()
 try { await page.getByText('✓ Glossary confirmed',{exact:true}).waitFor() } catch(error) { console.log('Confirmation diagnostics',await page.locator('body').innerText());throw error }
 await page.getByRole('link',{name:'Workspace',exact:true}).click()
 await page.getByRole('button',{name:'Run',exact:true}).click()
