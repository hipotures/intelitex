...
test('real Vite proxy preserves same-origin mutations and streams SSE without buffering', {timeout:30000}, async t => {
 ...
 const workspaces=await page.evaluate(async()=>await(await fetch('/api/workspaces')).json())
 assert.ok(workspaces.workspaces.find(w=>w.source_id==='second-book' && !w.prepared))
