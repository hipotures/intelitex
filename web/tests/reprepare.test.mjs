...
test('Prepare rebuild is explicit and F/T/E responds before slow background reads', { timeout: 60000 }, async () => {
  ...
  try {
    ...
    try {
      ...
      await page.locator('.processing-switch button[data-mode-value="Translate only"]').first().click()
      await page.locator('.processing-switch.saving button[data-mode-value="Translate only"].active').first().waitFor({ timeout: 300 })
      ...
      await page.locator('.processing-switch.saving').waitFor({ state: 'hidden' })
      assert.equal(mutations.length, 1)
      await page.waitForTimeout(3800)
      assert.equal(globalReadsAfterPatch, 0, 'section processing does not refetch the global workspace list')
      assert.equal(previewReads, 0, 'source text is not fetched for the workspace table')
      await page.locator('.section-name').first().click()
      await page.locator('#section-mode').selectOption('full')
      await page.locator('#section-mode[aria-busy="true"]').waitFor()
      assert.equal(await page.locator('#section-mode').inputValue(), 'full',
        'the drawer immediately shows the pending choice')
      await page.getByRole('dialog', { name: 'ONE' }).getByRole('alert').filter({ hasText: 'Configuration changed.' }).waitFor()
      assert.equal(await page.locator('#section-mode').inputValue(), 'translate',
        'a conflict restores the authoritative choice in the drawer')
      await page.getByRole('button', { name: 'Close' }).click()
      const readsAfterDrawer = previewReads
      await page.getByRole('button', { name: /Prepare.*Source structure frozen/ }).click()
      assert.equal(previewReads, readsAfterDrawer, 'Prepare does not fetch source text before selection')
