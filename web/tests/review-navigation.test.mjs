...
test('Review Next saves once, advances without waiting for workspace refresh, and protects failed drafts', { timeout: 90000 }, async t => {
  ...
  await page.route('**/api/workspaces/prepared/review/bulk-review', async route => {
    ...
    await route.fulfill({ json: { revision: 'bulk-test-revision', terms: [{ ...fakeSecond, reviewed: true, review_method: 'bulk' }],
  ...
  await page.locator('#reviewNextBtn').getByText('Review individually & next').click()
  ...
  assert.equal(await page.locator('.review-term-row').count(), 1, 'individually reviewed term leaves the Pending filter')
