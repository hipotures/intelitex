...
export function SourceDetails({ source, close, add }: { source: Source; close: () => void; add: () => void }) {
  ...
  return <Overlay drawer title={source.title} eyebrow="Library source" debugId="LSD" close={close}>
    <div className="drawer-body source-details"><div className="source-detail-head"><Cover title={source.title} large /><div><div className="source-detail-title">{source.title}</div><div className="subtitle">{source.creators.join(', ') || 'Unknown author'}</div><div className="subtitle">{source.language ? `Declared language: ${source.language}` : 'Language not declared'}</div></div></div>
      ...
      {inspecting && <section className="source-inspect" {...debugTag('LSI')}><h3>Source inspection</h3><ErrorNote error={inspect.error} retry={() => void inspect.refetch()} />{inspect.isPending ? <Empty>Inspecting source locally…</Empty> : inspect.data && <><p>{inspect.data.document_count ?? '—'} spine documents · {inspect.data.sample_word_count ?? '—'} words in {inspect.data.sampled_documents ?? '—'} sampled documents</p><p>Detected language: {inspect.data.detected_language ?? 'uncertain'}{inspect.data.detected_language ? ` (${Math.round(inspect.data.detection_confidence * 100)}% separation)` : ''}</p>{inspect.data.language_warning && <p className="notice">{inspect.data.language_warning}</p>}<ul>{inspect.data.section_preview?.map(name => <li key={name}>{name}</li>)}</ul></>}</section>}
    </div>
  </Overlay>
}
