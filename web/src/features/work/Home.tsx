...
export function WorkspaceRow({ workspace }: { workspace: Workspace }) {
  ...
  return <div className="workspace-row" onClick={e => { if (!(e.target as HTMLElement).closest('button,a')) e.currentTarget.querySelector<HTMLAnchorElement>('a')?.click() }}>
...
export function Home() {
  ...
  return <main className="main"><div className="page-title-row"><div><div className="eyebrow">Work</div><h1>Books in progress</h1><div className="subtitle">Open a workspace, continue the pipeline, or start working on a book from the library.</div></div></div>
    <ErrorNote error={workspaces.error ?? command.error} retry={() => void reconcile()} />
    <section className="section"><div className="section-header"><div className="section-title"><h2>Active workspaces</h2><span className="count-badge">{workspaces.data ? active.length : '—'}</span></div><Button variant="ghost" className="compact" onClick={() => setArchive(true)}>Archive →</Button></div>
      <div className="workspace-list">{active.map(w => <WorkspaceRow key={w.workspace_id} workspace={w} />)}{!active.length && <Empty>{workspaces.isPending ? 'Loading workspaces…' : 'No active workspaces.'}</Empty>}</div></section>
    <section className="section"><div className="section-header"><div className="section-title"><h2>Library</h2><span className="count-badge">{library.data ? sources.length : '—'}</span></div><div className="subtitle">Source directory: {library.data?.configured ? 'Configured' : 'Not configured'}</div></div>
      <ErrorNote error={library.error} retry={() => void library.refetch()} />
      <div className="library-grid">{sources.map(source => <button className="book-card" data-source-id={source.source_id} key={source.source_id} onClick={() => void openSource(source)} disabled={command.disabled} aria-label={`Open ${source.title}`}><Cover title={source.title} large /><div className="book-meta"><div className="book-name">{source.title}</div><div className="book-detail"><span>{source.creators.join(', ') || '—'}</span><span>·</span><span>{source.word_count?.toLocaleString() ?? '—'} words</span>{source.workspace_id && <span className="workspace-chip">workspace</span>}</div></div></button>)}</div>
    ...
    {archive && <Overlay drawer title="Archive" eyebrow="Workspaces" close={() => setArchive(false)}><div className="drawer-body"><p className="subtitle">Completed or parked workspaces stay available here.</p><div className="archive-list">{archived.map(w => <div className="archive-item" key={w.workspace_id}><div><strong>{w.metadata.title}</strong><ArchivedProgress workspace={w} /></div><Button disabled={command.disabled} onClick={async () => { if (await command.send(endpoint(w.workspace_id, 'restore'), lifecycleSchema, { revision: w.metadata.lifecycle.revision })) { setArchive(false); announce('Workspace restored.') } }}>Restore</Button></div>)}</div>{!archived.length && <Empty>Archive is empty.</Empty>}<ErrorNote error={command.error} /></div></Overlay>}
  </main>
}
