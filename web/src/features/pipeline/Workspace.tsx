...
export function Activity({ id, expanded = false, publicationOnly = false, title }: { id: string; expanded?: boolean; publicationOnly?: boolean; title?: string }) {
  ...
  return <section className={title ? 'phase-detail-card' : 'accordion'}><button className={title ? 'phase-detail-card-head activity-card-head' : 'accordion-head'} onClick={() => { setOpen(!open); following.current = true; savePreference(scope, `${id}.activity`, String(!open)) }} aria-expanded={open}>
...
export function WorkspacePage() {
  ...
  return <main className="main workspace-page"><Back /><div className="workspace-head"><div className="workspace-heading"><Cover title={workspace.metadata.title} /><div><div className="eyebrow">Workspace</div><h1>{workspace.metadata.title}</h1><div className="workspace-meta-line"><span>{workspace.metadata.creators.join(', ') || '—'}</span><span>·</span><span>{workspace.metadata.language ?? '—'} → PL</span><span>·</span><span>{workspace.metadata.word_count?.toLocaleString() ?? '—'} words</span></div></div></div><div className="workspace-head-actions"><Action workspace={workspace} pipeline={p} /></div></div>
    ...
    <div className="phase-rail">{(['prepare','analyse','review','translate','publish'] as const).map(phase => {
      const state = p ? phaseStatus(p, phase) : phase === 'prepare' ? 'active' : 'blocked'
    ...
    {!workspace.prepared ? <div className="prepare-card"><div className="prepare-icon">P</div><h2>Prepare this workspace</h2><p className="subtitle">Inspect the source and freeze its sections and translation units before running analysis.</p><Action workspace={workspace} /></div> : !p ? <Empty>Loading source structure…</Empty> : <>
