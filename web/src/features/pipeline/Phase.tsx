...
import { Back, Button, Empty, ErrorNote, Overlay, Panel } from '../../components/ui/common'
...
export function PhasePage() {
  ...
  return <main className={`main phase-detail-page${phase === 'prepare' ? ' prepare-detail-page' : ''}${phase === 'analyse' ? ' analyse-detail-page' : ''}`} {...debugTag('PHD')}><Back id={id} /><div className="phase-detail-head" {...debugTag('PHH')}><div className="phase-detail-title"><div className="eyebrow">{title} · {p?.metadata.title ?? workspace?.metadata.title ?? 'Workspace'}</div><h1>{phase === 'analyse' ? 'Analyse · P1' : title}</h1><div className="phase-detail-meta">{phase === 'analyse' ? `Whole-book analysis · ${p?.progress.analysis.required ? `${p.progress.analysis.completed}/${p.progress.analysis.required} required units` : 'plan not created yet'} · ${p?.review.summary?.total ?? '—'} entities` : 'Phase details and execution diagnostics'}</div></div><span className={`phase-status-badge ${state}`}>{state === 'done' ? 'Complete' : state === 'blocked' ? 'Blocked' : state === 'error' ? phase === 'translate' ? 'Previous run failed' : 'Failed' : p?.active_job ? 'Running' : 'Ready'}</span></div><ErrorNote error={query.error ?? usage.error ?? profiles.error} retry={() => void query.refetch()} />
