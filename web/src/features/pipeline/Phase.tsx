...
export function PhasePage() {
  ...
  const metrics = phase === 'prepare' ? [['Format',p?.metadata.format ?? '—'],['Sections',p?.sections.length ?? '—'],['Processable',p?.sections.filter(s => s.processing !== 'excluded').length ?? '—'],['Words',p?.metadata.word_count?.toLocaleString() ?? '—']] : phase === 'publish' ? [['Status',p?.publication.state ?? '—'],['Format',p?.publication.current ? 'EPUB' : '—'],['Sections',p?.sections.filter(s => s.processing !== 'excluded').length ?? '—'],['Size',p?.publication.size_bytes == null ? '—' : `${p.publication.size_bytes.toLocaleString()} bytes`]] : phase === 'analyse' ? [...totals.map((t,i) => [labels[i],t.value == null ? '—' : t.value.toLocaleString()]), ['Cost',cost.text]] : totals.map((t,i) => [labels[i],measured(t)])
