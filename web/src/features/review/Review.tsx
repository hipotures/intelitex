...
export function matches(term: Term, query: string, category: string, status: string) {
  ...
  return text.includes(query.trim().normalize().toLowerCase()) && (category === 'All' || term.category === category) && (status === 'All' || (status === 'Reviewed' ? term.reviewed : !term.reviewed))
}
...
export function ReviewPage() {
  ...
  const selected = (draft ? terms.find(t => t.id === draft.id) : undefined) ?? filtered.find(t => t.id === search.term)
    ?? (status === 'Unreviewed' && !filtered.length ? terms.find(t => t.id === search.term) : undefined) ?? filtered[0]
  ...
  async function confirmBulk() {
    ...
    void queryClient.invalidateQueries({ queryKey: [scope, endpoint(id, 'pipeline')] })
    void queryClient.invalidateQueries({ queryKey: [scope, '/api/workspaces'], refetchType: 'none' })
    setReviewNotice(`${result.changed_count} ${result.changed_count === 1 ? 'term' : 'terms'} reviewed. Current choices and notes kept.`)
    setBulkPending(null)
  }
    ...
    try {
      ...
      if (review) {
        ...
        if (nextId) {
          ...
          setSearch({ term: nextId, ...(status === 'Reviewed' ? { status: 'Unreviewed' } : {}) }, true)
        } else {
          setReviewNotice('All matching terms reviewed.')
          if (status === 'Unreviewed') setSearch({ term: selected.id }, true)
        }
        return
      }
  ...
  const reviewScope = terms.filter(term => matches(term, search.q ?? '', category, status === 'Unreviewed' ? 'All' : status))
  const scopeComplete = status === 'Unreviewed' && !filtered.length && reviewScope.length > 0 && reviewScope.every(term => term.reviewed)
  ...
  return <main className="main review-page" {...debugTag('REV', id)}><Back id={id} /><div className="review-page-head" {...debugTag('RVH')}><div className="review-page-title"><div><div className="eyebrow">Review · {p?.metadata.title ?? 'Workspace'}</div><h1>Terminology Review</h1><div className="review-page-meta">{query.data ? `${reviewed}/${terms.length} reviewed · ${terms.length - reviewed} remaining · ${uncertain} uncertain` : 'Load the terminology draft to review its choices.'}</div></div></div><div className="review-heading-actions">{p?.approved && <span className="review-confirmed-badge">✓ Glossary confirmed</span>}<Button variant="primary" disabled={disabled || !query.data || reviewed !== terms.length || dirty || !p?.review.current} onClick={async () => { const current = currentReview(); if (current) await command.send(endpoint(id, 'review/confirm-and-approve'), approvalSchema, { revision: current._revision }) }}>{p?.approved ? 'Reconfirm glossary' : 'Confirm glossary'}</Button></div></div>
    ...
    {!p?.analysis.complete ? <Empty>{workspace && !workspace.prepared ? 'Prepare this workspace before Review.' : 'Complete whole-book analysis before Review.'}</Empty> : !p.review.prepared ? <Empty><p>The terminology draft is not prepared.</p><Button disabled={disabled} onClick={() => void command.send(endpoint(id,'review/prepare'), reviewSchema, {})}>Prepare review</Button></Empty> : <>
      <div className="review-toolbar" {...debugTag('RVF')}><input className="review-search" aria-label="Search terminology" placeholder="Search source, alias, translation, meaning…" value={searchInput} onCompositionStart={() => { composing.current = true }} onCompositionEnd={e => { composing.current = false; setSearchInput(e.currentTarget.value) }} onChange={e => setSearchInput(e.target.value)} /><div className="review-filter-row">{['All','Unreviewed','Reviewed'].map(value => <button className={`review-chip ${status === value ? 'active' : ''}`} key={value} disabled={disabled} onClick={async () => { if (await flush()) { setReviewNotice(null); setSearch({ status: value as typeof status }, true) } }}>{value}</button>)}</div></div>
      <div className="review-filter-row categories" {...debugTag('RVC')}>{['All', ...new Set(terms.map(t => t.category))].map(value => {
        ...
        const base = status === 'Unreviewed' ? terms.filter(term => matches(term, search.q ?? '', value, 'All')) : visible
      ...
      <div className="review-bulk-bar" {...debugTag('RVB')}><div><span>This view: {filtered.length} terms · {filtered.length - remaining} reviewed · {remaining} remaining.</span>{reviewNotice && <span className="review-bulk-notice" role="status">{reviewNotice}</span>}</div><Button disabled={disabled || remaining === 0} onClick={() => void openBulk()}>Review remaining in this view ({remaining})</Button></div>
      <div className="review-layout" {...debugTag('RVL')}><div className="review-term-card" {...debugTag('RFL')}><div className="review-term-scroll">{filtered.map(term => <button key={term.id} className={`review-term-row ${term.id === selected?.id ? 'active' : ''}`} disabled={disabled} onClick={async () => { if (await flush()) { setReviewNotice(null); setSearch({ term: term.id }, true) } }}><span className="review-term-title"><span>{term.source}</span><span className={`review-term-status ${term.reviewed ? 'reviewed' : ''}`}>{term.reviewed ? '✓ reviewed' : '○ pending'}</span></span><span className="review-term-sub">{term.category} · {effective(term)}</span></button>)}{!filtered.length && <Empty>{query.isPending ? 'Loading terminology…' : scopeComplete ? 'All terms in this scope are reviewed.' : 'No terms match the current filters.'}</Empty>}<div className="review-term-list-tail" /></div></div>
      ...
      </div><div className="review-detail-footer" {...debugTag('RVM')}><div className={`review-term-status ${selected.reviewed ? 'reviewed' : ''}`}>{selected.reviewed ? 'Reviewed' : 'Not reviewed'}</div><div className="review-footer-actions"><Button id="reviewPrevBtn" disabled={disabled || filtered[0]?.id === selected.id} onClick={() => void next(false,-1,'previous')}>{moving === 'previous' ? 'Saving…' : '← Previous'}</Button><Button id="reviewNextBtn" variant="primary" disabled={disabled || (selected.reviewed && !dirty && !nextPending)} onClick={() => void next(true)}>{moving === 'primary' ? 'Saving…' : 'Review & next'}</Button><Button id="reviewOnlyNextBtn" disabled={disabled || filtered.at(-1)?.id === selected.id} onClick={() => void next(false,1,'plain')}>{moving === 'plain' ? 'Saving…' : 'Next →'}</Button></div></div></div> : <Empty>Select a terminology item.</Empty>}</div></div>
    </>}
    {bulkPending && <Overlay title="Review remaining terms?" debugId="RBM" compact close={() => { if (!command.pending) setBulkPending(null) }}><div className="modal-body"><p>Review {bulkPending.ids.length} remaining {bulkPending.ids.length === 1 ? 'term' : 'terms'} in this view?</p><p className="subtitle">{bulkPending.scope}</p><p className="subtitle">Current candidate or custom forms and notes will be kept. This does not confirm the glossary or call a model.</p>{bulkUnknown && <p className="subtitle">The outcome is unknown. Reload the current Review state before another attempt.</p>}<ErrorNote error={command.error} /></div><div className="modal-foot"><Button disabled={command.pending} onClick={() => setBulkPending(null)}>Cancel</Button><Button variant="primary" disabled={disabled || bulkUnknown} onClick={() => void confirmBulk()}>Review {bulkPending.ids.length} {bulkPending.ids.length === 1 ? 'term' : 'terms'}</Button></div></Overlay>}
