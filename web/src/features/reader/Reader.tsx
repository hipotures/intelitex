...
export function ReaderPage() {
  ...
  return <main className="main"><div className="page-title-row"><div><div className="eyebrow">Reader</div><h1>Reading view</h1><div className="subtitle">Verified translated text. Your reading position is independent from Work.</div></div></div><ErrorNote error={all.error} /><div className="reader-shell"><nav className="reader-books" aria-label="Reading workspaces">{books.map(w => <Link key={w.workspace_id} to="/reader/$workspaceId" params={{ workspaceId: w.workspace_id }} className={`reader-book ${id === w.workspace_id ? 'active' : ''}`}><strong>{w.metadata.title}</strong><span>{w.metadata.creators.join(', ') || '—'}{w.metadata.lifecycle.archived ? ' · Archived' : ''}</span></Link>)}</nav><div className="reader-content">{chosen ? <ReadingBook key={id} id={id} /> : <Empty>{all.isPending ? 'Loading books…' : 'No translated books yet.'}</Empty>}</div></div></main>
}
