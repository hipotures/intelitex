...
On 2026-09-22 the task owner explicitly resolved D02, D03, D04, D05 and D08 in the implementation conversation. A later [product decision in issue #1](https://github.com/hipotures/intelitex/issues/1) refines and supersedes the earlier *D05 Library/draft flow*: Library remains a source catalog, `Add to workspace` opens setup before creation, Save creates a configured draft, and multiple workspaces may share one source. Its read-only preflight, source/language/profile setup, durable metadata and Prepare boundary are part of that target. The old one-source/one-draft rule survives only as an implementation fact about the current API, not as the approved target. D02, D03, D04 and D08 retain their approved policies unchanged.

The issue's separate `Library → draft workspace → Prepare flow` implementation item remains open until the code and tests satisfy the refined D05 policy. The supplied implementation contract is an immutable historical handoff, so its sections 5.2, 16.1 and V33-02 retain the superseded one-draft wording; read those sections through this decision register. The original v33 bytes also remain unchanged. `tests/test_web_production.py` and `docs/web/contract-map.md` describe current implementation evidence, not certification of the refined D05 flow.
