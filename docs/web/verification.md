...
| `uv run --group dev python -m pytest -q` | 812 passed, 2 dependency deprecation warnings |
| `node --test tests/*.cjs` | 31 passed |
| `npm --prefix web run typecheck` | Passed with strict TypeScript |
| `npm --prefix web run lint` | Passed |
| `npm --prefix web run test:unit` | 9 passed in 3 files; stream ordering/isolation, Unicode ranges, UI primitives |
| `npm --prefix web run build` | Passed; production assets emitted into ignored `web/dist` |
| `npm --prefix web run test:e2e` | 3 passed; full offline workflow, real Vite proxy/origin/SSE, 1,000 sections and 2,000 terms at 100 events/s |
| `npm --prefix web run test:visual` | 12 unmasked Work comparisons passed; 6 widths × dark/light, maximum differing pixels 0.0951% at pixelmatch threshold 0.15 |
...
or page error. Guarded smoke asserted zero console, page, HTTP and request failures.
